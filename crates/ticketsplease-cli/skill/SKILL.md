...
- **Exit codes are the API.** Gate on them:
  ...
  | 4 | ticket not found |
  | 5 | dependency cycle |
  | 6 | **conflict** — guard found a scope escape (under-declaration), or a declared-area overlap when overlaps are gated (`--strict` / `[guard] gate_collisions`); also a lost claim race, a held-claim release, or a `why` conflict. By default a plain overlap is a non-failing **WARN** (exit 0), not a 6. |
...
3. **Before merging, guard the branch:**
   ...
   ```
   - Exit `0` → the diff stays within its declared scope; it clears this **pre-merge filter**. Read `severity`: `ok` = no overlap; `warn` = a declared-area overlap with an open ticket was reported but does **not** gate (the expected state under parallel dispatch — glance at `collisions` and coordinate only if a sibling truly rewrites the same area). This is a partitioning check, not a substitute for your normal build/test gate — disjoint branches can still conflict semantically.
   - Exit `6` → a **CONFLICT**: a scope escape (`under_declared` — scopes whose files the branch edited but that fall outside its declared area; file-authoritative), or — only when you opt in with `--strict` / `[guard] gate_collisions` — a gated `collisions` overlap (each tagged `direct` = a real overlap, `transitive` = reverse-dep-only, or `shared` = both additive and non-gating). Fix an escape with `set --add-scope <scope>` (the guard's note prints the exact command) or add the file to `paths`; coordinate on a gated overlap; then re-guard. Under `--strict`, `guard --ignore-transitive` waves a transitive-only overlap back through. See `references/parallel-workflow.md` for the resolution playbook and the `--direct-only` flag.
...
- `ticketsplease why <a> <b>` — explain whether two tickets can co-run, and if not, the exact reason (a shared exclusive scope, a dependency, or — when `[scheduler].related_weight` is positive — a related link). Use it when the scheduler's grouping is surprising.
...
**Batching keys on scope names, never on `paths`.** The scheduler — `tracks`, `why`, `lanes`, `next` — reasons about `scopes`/`shared_scopes`, plus an opt-in `related` cost. The `paths` field is read *only* by `guard` (as an under-declaration allowance); it looks like a file-intent declaration but does not feed the conflict math. A ticket that declares `paths` but no scopes is therefore **invisible to batching** and will be co-scheduled with work that rewrites the same files — so always give a ticket a `scope`, not just `paths`. `lint` flags the omission as `paths-without-scopes`.

`related` never gates `ready` and is never cycle-checked. With the default `[scheduler] related_weight = 0` it is also ignored by `tracks`/`next`/`lanes`/`why` (queryable and graphable only). Set `related_weight` to a positive integer to add that cost to each related pair; `--max-overlap` then treats it like a scope weight. `--related-weight N` overrides the config for one invocation.

For the full tuning playbook — scope weights (`[scope_policy]`), related coupling, the escape hatches, sequencing conflicts with `lanes`, and how the guard honours all of it — read `references/parallel-workflow.md`.
...
`dependencies` block scheduling; `related` is a soft, non-blocking "see also" (queryable via `--where related:x`, never cycle-checked, ignored by `ready`). `tracks`/`next`/`lanes` ignore it unless `[scheduler].related_weight` is positive. `--template <name>` scaffolds the body from `.ticketsplease/templates/<name>.md` (seeded by `init`). Add `--dry-run` to `create`/`set` to preview without writing. Edits are **round-trip-safe**: ticketsplease rewrites only the field it changes and leaves everything else — custom frontmatter keys, comments, key order, and the markdown body — byte-for-byte. You can also hand-edit ticket files directly; they are just markdown.
