...
1. **Emit the batch in one operation.** Write the tickets as a JSON array or a TOML `[[ticket]]` manifest and `ticketsplease create --from manifest.toml` — **transactional** (plan final ids → validate post-image → journaled commit; on failure nothing is written), idempotent on re-run, and `--dry-run` shows the real final ids. **Set an explicit `id` on every ticket that another member references** in `depends_on` / `related` (edge strings are literal; auto-id peers that lose the base slug to a collision are not retargeted). Tag every ticket with the initiative key (e.g. `tags = ["m1"]`) so the group can be rolled up. Use `depends_on` only for true ordering; `related` records a non-blocking cross-reference that never gates `ready` and is ignored by `tracks` unless `[scheduler].related_weight` is positive. Multi-step `tkt run` recipes are **not** cross-step transactional — each step is its own command.
...
- `ticketsplease comment add <id> --as <w> --body -` / `comment list <id>` — leave durable notes on a ticket (blocked-reasons, decisions, questions) and read the union of the worktree plus matching `tkt/<id>` tip from `main`. Use exact `--ref` or `--comment-source` only when isolating provenance. Each `comment add` also rings the event doorbell above.
...
- `ticketsplease show <id>` — read one ticket plus the deduplicated worktree/branch comment thread; ticket collections expose `comment_count`. Add `--ref tkt/<id>` for an exact committed branch view.
